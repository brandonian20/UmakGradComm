...
class GraduatesController extends Controller
{
            ...
            return  DataTables::of($data)
                ->editColumn('pictureID', function ($row) {
                    ...
                    if ($row['pictureID'] != null) {
                        ...
                        //Upload toga box
                        $data .=    "<div class='upload-box'>
                                        <input class='d-none' type='file' accept='image/*' id='img-toga-{$row['studentID']}'>
                                        <div data-student-id='" . Crypt::encryptString($row['studentID']) . "' data-img-id=" . Crypt::encryptString($row['image']) . " class='d-flex align-items-center h-100 text-center justify-content-center toga-box-upload'>
                                            <div><i class='fa-solid fa-download'></i> Toga</div>
                                        </div>
                                    </div>";
                    }
                    ...
                    if ($row['bannerImageID'] != null) {
                        ...
                        //Upload slide deck box
                        $data .=    "<div class='upload-box'>
                                        <input class='d-none' type='file' accept='image/*' id='img-slide-deck-{$row['studentID']}'>
                                        <div data-student-id='" . Crypt::encryptString($row['studentID']) . "' data-img-id=" . Crypt::encryptString($row['image']) . " class='d-flex align-items-center h-100 text-center justify-content-center slide-deck-box-upload'>
                                            <div><i class='fa-solid fa-download'></i> Slide Deck</div>
                                        </div>
                                    </div>";
                    }
    ...
    public function uploadToga(Request $r)
    {
        ...
        try {
            ...
            // return response()->json(["success" => false, 'data' => $r->hasFile('image')], 200);
    ...
    public function uploadSlideDeck(Request $r)
    {
